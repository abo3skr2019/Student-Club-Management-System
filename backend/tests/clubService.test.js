...
describe("updateClubAdmin", () => {
  ...
  test("should throw error if club not found", async () => {
    ...
    await expect(
      clubService.updateClubAdmin(clubId, newAdminId)
  ...
  test("should throw error if new admin not found", async () => {
    ...
    await expect(
      clubService.updateClubAdmin(clubId, newAdminId)
    ).rejects.toThrow("New admin not found");
  });
  ...
  test("should throw error if user is already admin", async () => {
    ...
    await expect(
      clubService.updateClubAdmin(clubId, newAdminId)
    ).rejects.toThrow("User is already this club's admin");
  });
...
describe("findByAdmin", () => {
  ...
  test("should throw error if adminId not provided", async () => {
    await expect(clubService.findByAdmin()).rejects.toThrow(
      "adminId is required"
    );
  });
});
...
describe("updateClub", () => {
  ...
  test("should throw error if clubId not provided", async () => {
    await expect(clubService.updateClub()).rejects.toThrow(
      "clubId is required"
    );
  });
});
...
describe("deleteClub", () => {
  ...
  test("should throw error if clubId not provided", async () => {
    await expect(clubService.deleteClub()).rejects.toThrow(
      "clubId is required"
    );
  });
});
